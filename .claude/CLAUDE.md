...
| **Code syntax palette** (`--code-key`/`-string`/`-comment`/`-keyword`/`-number`/`-punct`) | F5DS publishes no code palette and no syntax-highlighting guidance at all. VS Code Dark+, as the pre-F5DS design and docs.nginx.com both use, adjusted only where contrast required it (`#808080` → `#8B9099`, since VS Code's own separator grey measures 4.42:1 on `--code-bg`). **Do not "fix" these to status hues** — that was tried and it broke two rules at once: status hues carry sentiment rather than token role, and `--emerald`/`--amber` were being rendered as text, which this site forbids. Ratios are in `tokens.css`; the print block inverts them to on-token dark values, because the code surface prints light. |
...
- **Top bar** — `--topbar-h` **52px**, matching the bar the site already shipped; 64px was tried and read as loose. It holds the branding block (logo only, **left-aligned**, hugging the start of the bar at every width), the **page heading**, the GitHub link and the dark-mode toggle. The brand ends in a **short centred rule** — `.topbar-brand::after`, 32px tall — not a full-height border, so the bar is not divided into two panes. Pinning the block to `--sidebar-w` with the logo centred inside it was tried and reverted: it spends 264px on a 99px logo and pushes the label a third of the way across the bar. The bar's bottom rule is a plain full-width `border-bottom`; a `.topbar::after` starting at `--sidebar-w` was also tried and reverted, since the console screenshots show the rule crossing the whole bar. The heading is **one line, and which line depends on the width**. Above 900px `.topbar-eyebrow` carries the site or tool label in H3 Regular at `--text-secondary` and `.mobile-breadcrumb` is hidden; at or below 900px they swap, because the drawer hides the sidebar's active-item marker and the page title becomes the useful line. So `.mobile-breadcrumb` is accurate to its name again — `index.js` and `migration-core.js` keep its text in step with the active view, and renaming it means touching both. A stacked eyebrow-over-title arrangement was tried and reverted. The label sits **outside** the branding block, to the right of the divider — it was once inside a 264px block beside the logo, where the 99px logo left too little room and a label like "Networking for Kubernetes" truncated.
