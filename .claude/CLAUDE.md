...
- **Top bar** — the branding block (logo only, **centred** in a block pinned to the navigation width so the page title beside it lines up with the content area), the **page heading**, the GitHub link and the dark-mode toggle. The bar's bottom rule is a `.topbar::after` starting at `--sidebar-w`, **not** a `border-bottom` — it must not run under the branding block, so that the brand and the sidebar beneath it read as one continuous column the way the console does. Both revert at ≤900px, where the navigation is a drawer and there is no column to align to. The heading is **one line, and which line depends on the width**. Above 900px `.topbar-eyebrow` carries the site or tool label in H3 Regular at `--text-secondary` and `.mobile-breadcrumb` is hidden; at or below 900px they swap, because the drawer hides the sidebar's active-item marker and the page title becomes the useful line. So `.mobile-breadcrumb` is accurate to its name again — `index.js` and `migration-core.js` keep its text in step with the active view, and renaming it means touching both. A stacked eyebrow-over-title arrangement was tried and reverted. The label sits **outside** the branding block, to the right of the divider: inside it, the 264px block leaves 232px, and the 99px logo plus a label like "Networking for Kubernetes" truncates.
