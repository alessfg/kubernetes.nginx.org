...
- **HAProxy Migration Tool** — Interactive guide for migrating from the HAProxy Kubernetes Ingress Controller (`haproxytech/kubernetes-ingress`) to the NGINX Ingress Controller
- **ingress2gateway** (`kubernetes-sigs/ingress2gateway`) — CLI tool to convert Ingress resources to Gateway API

Project characteristics:

- No build system or package manager. A dependency-free Node test suite lives in `test/` (`node --test`; also runs in CI via `.github/workflows/tests.yml`) covering analyzer regressions, value-conversion semantics, and page↔engine↔module wiring
...
assets/
  ...
  js/   shared.js         # chrome behavior: dark-mode toggle, sidebar drawer, copy-to-clipboard, copyright year (globals)
        ...
        migration-haproxy.js        # HAProxy SOURCE module: HAPROXY_VERSION, HAPROXY_MAPPINGS, YAML-subset + route-acl parsers, NIC generators, sample presets; defines window.MIGRATION_SOURCE
...
### Migration tools (`ingress-nginx-migration.html`, `haproxy-migration.html`)

- Both migration tools are linked from the landing page with relative paths (`href="ingress-nginx-migration.html"`, `href="haproxy-migration.html"`) so the links resolve identically when opened locally, in PR previews, and in production. Do not change them to absolute FQDNs — that only works in production and breaks local testing.
- Both pages run on the shared engine: the page's SOURCE module (`migration-<source>.js`) supplies mapping data + `parseInput`/`buildPlan`; `migration-core.js` owns rendering/nav/checklist. **The analyzer's mappings (`ANNOTATION_MAPPINGS` / `HAPROXY_MAPPINGS` in the source modules) and the static reference tables in the HTML must stay in agreement in both directions:** editing a mapping (or its generator) means updating the matching reference-table row — **including the example YAML in the expanded panel, which must match what the corresponding generator emits** — and editing a reference row whose construct the analyzer handles (there is a matching mapping entry) means updating the mapping/generator to match. **Exception:** reference rows for NIC-only features with no source-controller equivalent (the left cell reads "No direct equivalent") have no analyzer counterpart, so editing them needs no JS change — e.g. the ingress-nginx page's `apiKey` Policy row, since the community controller has no API-key annotation for the analyzer to map. A recurring bug is a hand-written example drifting from its still-correct generator; treat the generator as the source of truth and fix the example to match it.

#### Verifying analyzer changes (run `node --test`)

The analyzer is pure data — a source module's `parseInput` → `buildPlan` returns a plain `MigrationPlan` object with no DOM. The `test/` suite (dependency-free `node:test`, run with `node --test`, enforced in CI) loads each source module + `migration-core.js` in a vm context under a `window`/`document` stub (`test/lib/load.js`; `createElement`/`getElementById`/etc. return a chainable no-op element) and drives `MIGRATION_SOURCE.analyzer.parseInput`/`buildPlan`: `analyzer.test.js` runs every sample preset × strategy plus edge cases, `conversions.test.js` pins the semantic transforms where the controllers' defaults/units diverge, and `wiring.test.js` statically checks that every element id, `data-*` hook, mapping anchor, and preset the engine or page references actually exists on the other side. **Load-bearing gotcha the suite encodes:** `buildPlan` runs each generator in a `try/catch` that only `console.warn`s on failure, so a broken generator **silently drops its resource** from the output instead of throwing — the loader captures `console.warn` and tests assert the count stays zero (`node --check` catches syntax only). When adding mappings, extend the tests, and sanity-check generated `k8s.nginx.org/v1` field names against the `json:` tags in `nginx/kubernetes-ingress/pkg/apis/configuration/v1/types.go` to catch invalid CRD fields.

#### Migration tool ordering and structure rules (both pages)

- **Mapping rows** within each category table must be sorted alphabetically by the source-controller construct name (left column — community annotation, HAProxy annotation/ConfigMap key/CRD).
...
The HTML markup for these elements (the topbar/sidebar/banner DOM) is present in `index.html`, `ingress-nginx-migration.html`, and `haproxy-migration.html` and must stay structurally in sync — the shared CSS/JS keys off shared IDs/classes (`#sidebar`, `#sidebarBackdrop`, `#menuToggle`, `#darkToggle`, `.topbar`, `.event-banner`, `#copyright-year`, `#page-announce`).
...
- **HAProxy Migration Tool**: https://kubernetes.nginx.org/haproxy-migration.html
...
- Docs tree (pin the tag the tool documents, e.g. `v3.2.12`): https://github.com/haproxytech/kubernetes-ingress/tree/v3.2.12/documentation
- Machine-readable annotation registry: https://github.com/haproxytech/kubernetes-ingress/blob/v3.2.12/documentation/doc.yaml
- Annotation code ground truth: https://github.com/haproxytech/kubernetes-ingress/tree/v3.2.12/pkg/annotations
- CRD API types (two parallel groups): https://github.com/haproxytech/kubernetes-ingress/tree/v3.2.12/crs/api
- Docs site: https://www.haproxy.com/documentation/kubernetes-ingress/
...
**Critical rule:** Every annotation, ConfigMap key, CRD field, or feature documented in the migration tools MUST exist in the version referenced by the tool's "Version Reference" banner — this applies to the NIC side AND the source side (ingress-nginx, HAProxy). Before adding any NIC feature to a migration tool:
...
- The NIC version in `MigrationTool.NIC` also drives the **HAProxy tool's** banners and install commands — after bumping it, audit the NIC release notes for features that change HAProxy mappings too (`assets/js/migration-haproxy.js` + `haproxy-migration.html`).

#### HAProxy Kubernetes Ingress Controller release (HAProxy migration tool)

- Update the `HAPROXY_VERSION` constant at the **top of `assets/js/migration-haproxy.js`** (single source of truth for the Version Reference banner and release link on `haproxy-migration.html`).
- Update the static fallback text inside the `data-haproxy-version` spans / `data-haproxy-release-link` anchor in `haproxy-migration.html`.
- Audit the HAProxy IC release notes for new/renamed annotations, ConfigMap keys, or CRD fields and update `HAPROXY_MAPPINGS` + the reference tables accordingly. Verify against the `haproxytech/kubernetes-ingress` release tag (`documentation/doc.yaml` is the machine-readable annotation registry; `pkg/annotations/annotations.go` is the code ground truth).
