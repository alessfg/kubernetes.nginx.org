...
- **`.nojekyll`** bypasses the Jekyll build so files are served verbatim. Note what it does *not* do: it does not exclude anything. `scripts/`, `README.md` and `gallery.html` are all reachable over the Pages URL, and would be with Jekyll on too — its default config excludes none of them. Excluding a directory would need a `_config.yml`, which means turning Jekyll back on. Since the repository is public, nothing here is exposed that was not already. What `.nojekyll` buys is predictability: no `_`-prefixed path silently disappearing from the build, and no Jekyll interpretation of file contents.
- **Switching the fork's Pages source branch does not trigger a rebuild.** After `gh api -X PUT repos/alessfg/kubernetes.nginx.org/pages -f 'source[branch]=…'`, the previous commit stays live until you `gh api -X POST repos/alessfg/kubernetes.nginx.org/pages/builds`. Check with `gh api …/pages/builds/latest` and compare its `commit` against the branch tip.
