...
def html_files():
    ...
    # reason .github/ never reaches the site, so asserting anything about one
    # grades a file that cannot be published. shot.sh below writes .shot-tmp.html
    # into the repo root while it renders and removes it on exit, so a killed
    # render — or a check run that merely overlapped one — failed four assertions
    # at once, ending on "sitemap.xml does not list: .shot-tmp.html".
