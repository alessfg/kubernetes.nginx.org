...
#
# The four static checks cannot see a rendered page; AGENTS.md says so and then
...
#
# It refuses to pretend. If there is no browser it says so and exits non-zero,
# because the failure this exists to prevent is an agent reporting "verified
# visually" on the strength of a command that never rendered anything.
set -uo pipefail
