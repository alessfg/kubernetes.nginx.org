/* kn/migration.js — ported from assets/js/migration.js for the Hugo build.
   Chrome selectors repointed for the theme ('.topbar'->'.header', #sidebar->
   #knToolSidebar). copyToClipboard comes from kn/chrome.js. ANNOTATION_MAPPINGS
   and SAMPLE_PRESETS are build-injected from data/annotations.json & data/presets.json
   as window.KN_ANNOTATIONS / window.KN_PRESETS (see head_custom.html); crdInstall is
   recomputed from the version consts so it tracks releases. */
    ...
    (function() {
        ...
        const ANNOTATION_MAPPINGS = (window.KN_ANNOTATIONS || []).map(function(m) { if (m.nicMapping && m.nicMapping.crdKind) { m.nicMapping.crdInstall = NIC_CRD_INSTALL_CMD; } return m; });
        ...
        function chromeHeight() {
            let tb = document.querySelector('.header');
        ...
        // Sample YAML presets
        let SAMPLE_PRESETS = window.KN_PRESETS || {};
        ...
        (function() {
            ...
            function closeMobileSidebar() {
                let sidebar = document.getElementById('knToolSidebar');
