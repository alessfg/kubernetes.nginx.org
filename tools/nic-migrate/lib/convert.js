...
function buildIngress(model, gen, swaps, opts) {
    ...
    /* Session affinity maps to VirtualServer upstreams[].sessionCookie — see
       the mapping in migration-ingress-nginx.js, type "virtualserver". What it
       does not have is an Ingress annotation form, so the annotation strategy
       emits nothing for it and stickiness is silently lost on this target.
       The note must not mention Plus; nic-migrate.test.js asserts that. */
    if (model.communityAnnotations.affinity &&
        ...
        notes.push('session affinity (affinity: ' + model.communityAnnotations.affinity + ') is NOT ' +
            ...
            'which has no annotation equivalent. Use --target virtualserver, or accept the loss of ' +
            'stickiness.');
    }
